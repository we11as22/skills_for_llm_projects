...
python3 "$SKILLS_DIR"/langgraph-multi-agent-systems/scripts/scaffold_multi_agent_graph.py "$TMP_DIR/multi"
python3 "$SKILLS_DIR"/langgraph-rag-architectures/scripts/scaffold_rag_graphs.py "$TMP_DIR/rag"
python3 "$SKILLS_DIR"/kubernetes-platform-engineer/scripts/scaffold_k8s_service.py demo --output "$TMP_DIR/k8s"
...
PY
then
  python3 "$SKILLS_DIR"/llm-as-a-judge-designer/scripts/run_llm_judge.py \
    --task "Demo task" \
    --candidate "$TMP_DIR/candidate.txt" \
    --schema "$SKILLS_DIR/llm-as-a-judge-designer/assets/judge-output-schema.json" \
    --use-local-fallback \
